            ...
            <div class="card-body">
                <form action="{{route('admin.business-settings.order-settings.update-order-settings')}}" method="post" enctype="multipart/form-data" id="add_fund">
                    ...
                    <div class="row align-items-end">
                        ...
                        @php($minimum_order_qty_status=\App\CPU\Helpers::get_business_settings('minimum_order_qty_status'))
                        <div class="col-xl-4 col-md-6">
                            <div class="d-flex justify-content-between align-items-center gap-10 form-control form-group">
                                <span class="title-color">
                                    {{translate('minimum_order_qty')}}
                                    <span class="input-label-secondary cursor-pointer" data-toggle="tooltip" data-placement="top" title="{{translate('if_enabled_customers_must_place_at_least_or_more_than_the_order_qty_that_admin_or_sellers_set')}}">
                                        <img width="16" src="{{asset('/public/assets/back-end/img/info-circle.svg')}}" alt="">
                                    </span>
                                </span>

                                <label class="switcher" for="minimum_order_qty_status">
                                    <input type="checkbox" value="1" class="switcher_input" name="minimum_order_qty_status" id="minimum_order_qty_status" {{ $minimum_order_qty_status == 1 ? 'checked':'' }}
                                    onclick="toogleModal(event,'minimum_order_qty_status','minimum-order-amount-on.png','minimum-order-amount-off.png','{{translate('want_to_Turn_ON_Minimum_Order_qty')}}','{{translate('want_to_Turn_OFF_Minimum_Order_qty')}}',`<p>{{translate('if_enabled_customers_must_order_over_the_minimum_qty_of_orders_that_admin_or_sellers_set')}}</p>`,`<p>{{translate('if_disabled_there_will_be_no_minimum_order_restrictions_and_customers_can_place_any_order_qty')}}</p>`)">
                                    <span class="switcher_control"></span>
                                </label>
                            </div>
                        </div>
