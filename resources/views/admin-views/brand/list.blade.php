                    ...
                    <div class="px-3 py-4">
                        <div class="row g-2 flex-grow-1">
                            ...
                            <div class="col-sm-4 col-md-6 col-lg-8 d-flex justify-content-end">
                                <button type="button" class="btn btn-outline--primary ml-2" data-toggle="dropdown">
                                    <i class="tio-download-to"></i>
                                    {{translate('export')}}
                                    <i class="tio-chevron-down"></i>
                                </button>
                                <button type="button" class="btn btn-primary ml-2" data-toggle="dropdown"
                                        onclick="sendOrderToServer()">
                                    <i class="tio-filter-list"></i>
                                    {{translate('save_order')}}
                                </button>
                                <form action="{{ url()->current() }}" method="GET" class="d-inline-block ml-2">
                                    <select name="row_count" id="row_count" class="form-control" onchange="this.form.submit()">
                                        <option value="10" {{$row_count == 10 || !$row_count ? 'selected' : ''}}>{{translate('show')}} 10</option>
                                        <option value="20" {{$row_count == 20 ? 'selected' : ''}}>{{translate('show')}} 20</option>
                                        <option value="30" {{$row_count == 30 ? 'selected' : ''}}>{{translate('show')}} 30</option>
                                        <option value="40" {{$row_count == 40 ? 'selected' : ''}}>{{translate('show')}} 40</option>
                                        <option value="50" {{$row_count == 50 ? 'selected' : ''}}>{{translate('show')}} 50</option>
                                        <option value="100" {{$row_count == 100 ? 'selected' : ''}}>{{translate('show')}} 100</option>
                                        <option value="all" {{$row_count == $br->total() ? 'selected' : ''}}>{{translate('show')}} {{translate('All')}}</option>
                                    </select>
                                </form>
                            </div>
                        </div>
...
@push('script')
  <script src="{{asset('public/assets/back-end/js/jquery-ui.min.js')}}"></script>
    <script>
        ...
        function sendOrderToServer(){
            ...
            $.ajax({
                ...
                error: function(response){
                    toastr.error("Brand sort order updated failed");
                }
            });
        }
    </script>
