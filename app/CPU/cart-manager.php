...
class CartManager
{
    ...
    public static function add_to_cart($request, $from_api = false)
    {
        ...
        // check if the product max quantity is reached
        ...
        // check if user has more left to order
        $left = self::check_last_time_order($user->id,$product->id,$request['quantity']);
        $message = '';
        if($product->maximum_order_qty > 0 && $left==0){
            return [
                'status' => 0,
                'message' => translate('Maximum_order_for_this_product_is_') . $product->maximum_order_qty ." ". translate('only')
            ];
        }elseif($product->maximum_order_qty > 0 && $left> 0 && $left < $request['quantity']){
            $message = translate('Maximum_order_for_this_product_is_') . $product->maximum_order_qty ." ".  translate('only');
            $request['quantity'] = $left;
        }
    ...
    public static function update_cart_qty($request)
    {
        ...
        // check if user has more left to order
        if($product->maximum_order_qty > 0 && self::check_last_time_order($user->id,$product->id,$request['quantity']) < $request['quantity']){
            return [
                'status' => 0,
                'message' => translate('Maximum_order_for_this_product_is_') . $product->maximum_order_qty ." ".  translate('only')
            ];
    ...
    public static function check_last_time_order($user_id,$product,$qty)
    {
        $left = 0;
        // get product 
        $product = Product::where('id',$product)->first();
        $maximum_order_qty = $product->maximum_order_qty;
        $maximum_order_time	 = $product->maximum_order_time;
        if($maximum_order_qty && $maximum_order_qty > 0){

            if($qty > $maximum_order_qty){
                return $maximum_order_qty;
            }

            if($maximum_order_time){
            $max_date = date('Y-m-d H:i:s', strtotime('-'.$maximum_order_time.' hours'));
            ...
            // get all customer active carts
            $carts = self::get_cart_group_ids();    
            $carts = Cart::whereIn('cart_group_id',$carts)->get();
            $carts = $carts->groupBy('cart_group_id');
            // get total qty of this product in all carts
            $total_qty = 0;
            foreach($carts as $cart){
                $cart = $cart->where('product_id',$product->id);
                $total_qty += $cart->sum('quantity');
            }
            $left = $maximum_order_qty - $total_qty;
            
        }
        return $left;
    }

    public static function is_carts_valid($carts)
    {
        $valid = true;
        foreach ($carts as $cart) {
            $product = Product::find($cart['product_id']);
            $product_variation = json_decode($product->variation);
            $count = count($product_variation);
            if ($count) {
                for ($i = 0; $i < $count; $i++) {
                    dd($product_variation[$i]->type, $cart['variant']);
                    if ($product_variation[$i]->type == $cart['variant']) {
                        // check for qty
                        if ($product_variation[$i]->qty < $cart['quantity']) {
                            $valid = false;
                            //remove  cart
                            $cart->delete();
                        }
                        // check for price 
                        if ($product_variation[$i]->price != $cart['price']) {
                            $valid = false; 
                            //remove  cart
                            $cart->delete();
                        }
            ...
            } else if (($product['product_type'] == 'physical') && ($product['current_stock'] < $cart['quantity'] || $cart['price'] != $product->unit_price)) {
                $valid = false;
                $cart->delete();
            }
        }
    ...
    public static function cart_total_qty($cart_group_id = null)
    {
        $cart = CartManager::get_cart($cart_group_id);
        $total = 0;
        if (!empty($cart)) {
