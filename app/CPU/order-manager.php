...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        // process cashback check and processing
        $transaction_cashback = CustomerManager::create_cashback_transaction($order['customer_id'], $order_id, $or['order_amount'], 'order_place');
    ...
    public static function order_again($request)
    {
        ...
        foreach ($order_products as $key=>$order_product) {
            ...
            if($product) {
                $product_valid = true;
                if($product->maximum_order_qty > 0 && self::check_last_time_order($user->id,$order_product->product_id, $order_product->qty) == 0) {
                       $product_valid = false;
                }
    ...
    public static function minimum_order_qty_verify($request, $cart_group_id = null)
    {
        $user = Helpers::get_customer($request);
        $status = 1;
        $qty = 0;
        $minimum_order_qty = 0;
        $minimum_order_qty_status = Helpers::get_business_settings('minimum_order_qty_status');
        $inhouse_minimum_order_qty = Helpers::get_business_settings('minimum_order_qty');
        $decimal_point_settings = Helpers::get_business_settings('decimal_point_settings');

        if($minimum_order_qty_status) {
            $query = Cart::with(['seller', 'all_product'])
                ->where([
                    'customer_id' => ($user == 'offline' ? (session('guest_id') ?? $request->guest_id) : $user->id),
                    'is_guest' => ($user == 'offline' ? 1 : '0'),
            ...
            if ($cart_group_id) {
                $cart_item = $query->where('cart_group_id', $cart_group_id)->first();
                $minimum_order_qty = $inhouse_minimum_order_qty;

                $qty =  CartManager::cart_total_qty($cart_group_id);
                $status = $qty < $minimum_order_qty ? 0 : 1;
                ...
                foreach ($cart_groups as $group_key => $cart_group) {
                    ...
                    $new_qty = CartManager::cart_total_qty($group_key);
                    ($minimum_order_qty > $new_qty ? $status = 0 : '');
                    $qty = $qty + $new_qty;
                }
            }
        }
        ...
        $data = [
            'minimum_order_qty'=> $minimum_order_qty ?? 0,
    ...
    public static function check_last_time_order($user_id,$product,$qty)
    {
        ...
        if($maximum_order_qty && $maximum_order_qty > 0){
            ...
            $carts = CartManager::get_cart_group_ids();    
