...
class SystemController extends Controller
{
    ...
    public static function check_shipping_minimum_order($request)
    {
        ...
        $cart_total = CartManager::cart_total(Cart::whereIn('cart_group_id', $request['cart_group_id'])->first());
        $shipping_method = ShippingMethod::find($request['id']);
        if ($shipping_method->min_order > $cart_total) {
            return translate('Minimum_order_for_this_shipping_method_is_') . $shipping_method->min_order;
        }else{
            return 'ok';
        }
    }
