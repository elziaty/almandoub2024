...
class CartController extends Controller
{
    ...
    public function cart(Request $request)
    {
        ...
        if($cart) {
            ...
            foreach($cart as $data) {
                ...
                $data['minimum_order_qty_info'] = OrderManager::minimum_order_qty_verify($request, $data['cart_group_id']);
    ...
    public function refresh_cart(Request $request)
    {
        ...
        $check_cart_validity = CartManager::is_carts_valid($carts);
        if(!$check_cart_validity){
            return response()->json(['status'=>'fail', 'message' => translate('cart_refreshed_due_to_a_problem')], 403);
        }
