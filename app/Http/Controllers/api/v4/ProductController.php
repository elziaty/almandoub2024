...
class ProductController extends Controller
{
    ...
    public function scan_barcode_products(Request $request)
    {
        try{
            $products = Product::where('code', $request->barcode)->limit(1)->get();
            if($products->count() == 0){
                return response()->json(['status' => true,'message' => translate('No product found') , 'data' => []], 404);
            }
            ...
            \Log::error($e);
            return response()->json(['status' => false, 'message' => translate('Something went wrong')], 500);
        }
    }
}
