...
class BusinessSettingsController extends Controller
{
    ...
    public function updateInfo(Request $request)
    {
        ...
        //Deal of the day
        BusinessSetting::updateOrInsert(['type' => 'deal_of_the_day'], [
            'value' => $request['deal_of_the_day']
        ]);

        BusinessSetting::updateOrInsert(['type' => 'sellers_page'], [
            'value' => $request['sellers_page']
        ]);
    ...
    public function update_fcm_messages(Request $request)
    {
        ...
        DB::table('business_settings')->updateOrInsert(['type' => 'not_completed_order_message_title'], [
            'value' => json_encode([
                'message' => $request['not_completed_order_message_title'],
            ]),
        ]);
        
        DB::table('business_settings')->updateOrInsert(['type' => 'not_completed_order_message_description'], [
            'value' => json_encode([
                'message' => $request['not_completed_order_message_description'],
            ]),
        ]);
    ...
    {
        return view('admin-views.business-settings.feedback_categories');
    }

    public function feedback_categories_fetch(Request $request)
    {
        if ($request->ajax()) {
            $data = FeedbackCategory::orderBy('id', 'desc')->get();

            $data->map(function($feedback_categories){
                $feedback_categories['title'] = translate($feedback_categories['title']);
            });
    ...
    public function feedback_categories_store(Request $request)
    {
        $check = FeedbackCategory::where('title', $request->title)->first();
        if ($check != null) {
            return response()->json([
                'error' => 1,
            ]);
        ...
        $feedback_categories = new FeedbackCategory;
        $feedback_categories->title = $request->title;
        $feedback_categories->save();
        return response()->json([
            'success' => 1,
        ]);
    }

    public function feedback_categories_edit(Request $request)
    {
        $data = FeedbackCategory::where('id', $request->id)->first();
        return response()->json($data);
    }

    public function feedback_categories_update(Request $request)
    {
        $feedback_categories = FeedbackCategory::find($request->id);
        $feedback_categories->title = $request->title;
        $feedback_categories->save();
        return response()->json();
    }

    public function feedback_categories_delete(Request $request)
    {
        $br = FeedbackCategory::find($request->id);
        $br->delete();
        return response()->json();
    }
    ...
    {
        return view('admin-views.business-settings.feedback_responses');
    }

    public function feedback_responses_fetch(Request $request)
    {
        if ($request->ajax()) {
            $data = Feedback::orderBy('id', 'desc')->get();

            $data->map(function($feedback_responses){
                $feedback_responses['title'] = translate($feedback_responses['title']);
                $feedback_responses['category'] = translate($feedback_responses['category']);
                $feedback_responses['description'] = translate($feedback_responses['description']);
            });
    ...
    public function feedback_responses_show(Request $request)
    {
