...
class BrandController extends Controller
{
    ...
    function list(Request $request)
    {
        ...
        $row_count   = $request['row_count'] ?? Helpers::pagination_limit();
        $row_count   = $row_count == 'all' ? Brand::count() : $row_count;
        ...
        return view('admin-views.brand.list', compact('br','search','row_count'));
    }
    ...
    public function sort(Request $request)
    {
        try{
            ...
            foreach ($orders as $row) {
                Brand::where('id', $row['id'])->update(['sort_order' => $row['position']]);
            }
            return response()->json([
                'success' => true,
                'message' => translate('brand_sort_order_updated_successfully')
            ], 200);
        }catch (\Exception $e){
            return response()->json([
                'success' => false,
                'message' => translate('brand_sort_order_updated_failed')
            ], 403);
        }
    }
}
