...
class InhouseShopController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->has('minimum_order_qty')) {
            DB::table('business_settings')->updateOrInsert(['type' => 'minimum_order_qty'], [
                'value' => BackEndHelper::currency_to_usd($request['minimum_order_qty']) ?? 0
            ]);
        }
        ...
        if ($request->has('deal_of_the_day')) {
            DB::table('business_settings')->updateOrInsert(['type' => 'deal_of_the_day'], [
                'value' => BackEndHelper::currency_to_usd($request['deal_of_the_day']) ?? 0
            ]);
        }
